...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine",mask = None,deform = True):
    """
    Render the scene. 
    ...
    # print(deformation_point.shape,type(deformation_point),"deformation_point")
    if stage == "coarse" :
    ...
    # print(time.max())
        ...
        # print(torch.max(opacity_deform).item(),torch.sum(opacity_deform).item()) 
    ...
    # if(mask is not None): print(torch.max(opacity_final).item(),torch.sum(opacity_final).item(),deformation_point.sum().item())
    ...
    if mask is not None: #the masked value be 0
        # print(torch.max(opacity_deform).item(),torch.sum(opacity_deform).item()) 
        opacity[~mask] = 0
        opacity_final[~mask] = 0
    # print(opacity.max())
...
def point_cloud_render(
    ...
    rendered_image, radii, depth = rasterizer(
        means3D=means3D.to(torch.float),
        means2D=means2D.to(torch.float),
        shs=shs,
        colors_precomp=colors_precomp,
        opacities=opacity.to(torch.float),
        scales=scales.to(torch.float),
        rotations=rotations.to(torch.float),
        cov3D_precomp=cov3D_precomp,
    )
