...
class GaussianEditor(BaseLift3DSystem):
    ...
    def configure(self) -> None:
        self.gaussian = GaussianModel(
            ...
            args = torch.load("/home/xshe/4D/jumping.pth"),
        )
    ...
    @torch.no_grad()
    def update_mask(self, mask,t) -> None:
        ...
        cur_cam = self.trainer.datamodule.train_dataset.scene.cameras[0]

        mask = self.text_segmentor(self.origin_frames[0], self.cfg.seg_prompt)[
                    0
                ][0].to(get_device())
        ...
        #         # print(masked_image.shape, mask.shape, "masked_image")
        ...
        if True: 
            ...
            total_sum = weights.sum().item()
            threshold = total_sum * 0.8
            flat_tensor = weights.view(-1)
            sorted_tensor, indices = torch.sort(flat_tensor, descending=True)


            cumulative_sum = torch.cumsum(sorted_tensor, dim=0)
            n = torch.searchsorted(cumulative_sum, threshold).item() + 1
            # print(n)


            smask = torch.zeros_like(flat_tensor, dtype=torch.bool)
            smask[indices[:n]] = True

            # mask = mask.view(tensor.shape)
            # print(weights.sum(),weights.mean(), "weights",selected_mask.sum(),mask.sum()) 
    ...
    def forward(self, batch: Dict[str, Any], renderbackground=None, local=False,mask=None,deform = True) -> Dict[str, Any]:
        ...
        # print(batch["camera"], "batch_camera")
        for id, cam in enumerate(batch["camera"]):
            render_pkg = render(cam, self.gaussian, self.pipe, renderbackground,mask=mask,deform = deform)
    ...
    def on_test_epoch_end(self):
        ...
        self.gaussian.save_ply(f"outputs/{current_time}.ply")
        self.gaussian.save_deformation("outputs")
    ...
    def configure_optimizers(self):
        ...
        self.gaussian.load_model(self.cfg.gs_source)
        self.gaussian.load_ply(os.path.join(self.cfg.gs_source, "point_cloud.ply"))
        #load deform model
        
        print("load ply",self.cfg.gs_source)
