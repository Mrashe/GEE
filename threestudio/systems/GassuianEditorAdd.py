...
class GaussianEditor_Add(GaussianEditor):
    ...
    def on_fit_start(self) -> None:
        ...
        if self.cfg.cache_overwrite or not os.path.exists(mesh_path):
            print(
                f"{sys.prefix}/bin/python launch.py --config configs/neuralangelo-ortho-wmask.yaml --save_dir {align_cache_dir} --gpu 1 --train dataset.root_dir={os.path.dirname(mv_image_dir)} dataset.scene={os.path.basename(mv_image_dir)}"
            )
            cmd = f"{sys.prefix}/bin/python launch.py --config configs/neuralangelo-ortho-wmask.yaml --save_dir {align_cache_dir} --gpu 1 --train dataset.root_dir={os.path.dirname(mv_image_dir)} dataset.scene={os.path.basename(mv_image_dir)}".split(
                " "
            )
            p2 = subprocess.Popen(
                cmd,
